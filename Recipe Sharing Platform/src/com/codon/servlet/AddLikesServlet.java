...
public class AddLikesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!IDao.isLiked(Recipe_ID, User_Name)) {
			boolean success = IDao.addLikes(Recipe_ID, User_Name);
			if(success && User_Name!=null) {	
		        ...
		        try {
		            ...
		            message1.setSubject("Recipe Liked");
		            
		            String Content = "<html><body style=\"font-family: Arial, sans-serif; color: #333;\">"
		            	    + "<div style=\"background-color: #f8f9fa; padding: 20px; border-radius: 10px;\">"
		            	    + "<h2 style=\"color: #007bff; text-align: center;\"> Your Recipe Just Got a Like! </h2>"
		            	    + "<p style=\"font-size: 16px;\">Hello,</p>"
		            	    + "<p style=\"font-size: 16px;\">We are excited to inform you that <strong>" + User_Name 
		            	    + "</strong> has just liked your recipe, <strong>" + RecipeName + "</strong>, on our Recipe Sharing Platform!</p>"
		            	    + "<p style=\"font-size: 16px;\">Your culinary creation is gaining popularity, and we're thrilled to see it being appreciated.</p>"
